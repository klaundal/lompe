...
class Emodel(object):
    def __init__(self, grid,
        ...
        # Matrices to evaluate electric field on inner grid:
...
def save_model(model, save='all', 
    ...
    **kwargs : dict
    ...
    ds1.attrs['Data_locs']= str(data_locs)
    ds1.attrs['Epoch']= model.epoch
    ds1.attrs['Dipole']=str(model.dipole)
    ds1.attrs['grid_info_E']=str(model.grid_E.to_dictionary())
    ds1.attrs['grid_info_J']=str(model.grid_J.to_dictionary())
...
def load_model(file, time='first'):
    ...
    ds= ds.sel(time=time)
    grid= cs.from_dictionary(eval(ds.attrs['grid_info_E']))
    model=Emodel(cs.from_dictionary(eval(ds.attrs['grid_info_J'])),[partial(interp, grid, ds['hall'].values), 
                                        partial(interp, grid, ds['pedersen'].values)], epoch= float(ds.Epoch),
                 dipole= eval(ds.Dipole))
    model.m= ds.model_vector.values.flatten()
    if eval(ds.attrs['Data_locs']):
...
def load_grid(file):
    ...
    return cs.from_dictionary(eval(ds.attrs['grid_info_J'])), cs.from_dictionary(eval(ds.attrs['grid_info_E']))
