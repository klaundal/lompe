...
def sm_DateToYMDHMS(tval, yr, mo, dy, hr, mt, sc):
    ...
    datestr = pd.to_datetime(julday, unit='D', origin='julian')
    return (datestr)


def sm_keycheck_data(flagstring):
    # internal helper function
    toggles = ['mlt', 'mag', 'geo', 'decl', 'sza', 'delta=start',
               'baseline=daily', 'baseline=yearly', 'baseline=none']
...
def sm_keycheck_indices(flagstring):
    ...
    #
    basekeys = ["sme", "sml", "smu", "mlat",
                "mlt", "glat", "glon", "stid", "num"]
    # sunkeys: alias allowed of SUN___ -> ___s
    sunkeys = ["smes", "smls", "smus", "mlats",
               "mlts", "glats", "glons", "stids", "nums"]
    # darkkeys: alias allowed of DARK___ -> ___d
    darkkeys = ["smed", "smld", "smud", "mlatd",
                "mltd", "glatd", "glond", "stidd", "num"]
    # regkeys: alias allowed of REGIONAL___ -> ___r
    regkeys = ["smer", "smlr", "smur", "mlatr",
               "mltr", "glatr", "glonr", "stidr", "numr"]
    ...
    swikeys = ["pdyn", "epsilon", "newell", "clockgse",
               "clockgsm", "density"]  # % or swiall for all these
    myflags = ''
    indices = '&indices='
    swi = '&swi='
    imf = '&imf='
...
def sm_microtest(choice, userid):
    ...
    if choice == 2 or choice == 4:
        (status, data) = SuperMAGGetData(userid, start,
                                         3600, 'all,delta=start,baseline=yearly', 'HBK')
        print(status)
        print(data)
        print(data.keys())
...
def supermag_testing(userid):
    ...
    # Index(['tval', 'ext', 'iaga', 'N', 'E', 'Z'], dtype='object')
    mydata1a.keys()
    ...
    # Index(['tval', 'ext', 'iaga', 'glon', 'glat', 'mlt', 'mcolat', 'decl', 'sza', 'N', 'E', 'Z'], dtype='object')
    mydata1a.keys()

    # BARE CALL, list returned
    (status, mydata1b) = SuperMAGGetData(
        userid, start, 3600, '', 'HBK', FORMAT='list')
    ...
    (status, idxdata) = SuperMAGGetIndices(
        userid, start, 3600, 'all,swiall,imfall')
    ...
    # item is a pandas series (not python list)
    stationlist = mydata2b.SMLrstid
    ...
    stationlist2 = sm_csvitem_to_list(
        mydata2b.SMLrstid)  # goal is a list of stations
    ...
    vgse_xyz = [(mydat['X'], mydat['Y'], mydat['Z'])
                for mydat in vgse]  # grab all 3
    ...
    result = [(myeach['tval'], myeach['density'])
              for myeach in mydata2c]  # pull out pairs e.g. 'tval, density')
    # two-line method for extracting any variable set from this
    # same, pull out pairs, only assign e.g. x=tval, y=density
    pairsets = [(myeach['tval'], myeach['density'], myeach['vgse'])
                for myeach in mydata2c]
    ...
    pairsets = [(myeach['tval'], myeach['density'], myeach['vgse']['X'])
                for myeach in mydata2c]  # same, pull out pairs, only assign e.g. x=tval, y=density
