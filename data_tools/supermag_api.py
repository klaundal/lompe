...
def sm_GetUrl(fetchurl,fetch='list'):
  # internal helper function
  # returned data choices are 'raw', 'list' or 'json', default is 'list'
  # converts an http bytestream into a python list (list)
  # or list of dict (json) or just return the raw webfetch (raw)
  # 'stations' should be 'list' and returns a list
  ...
  try:
    ...
    with urllib.request.urlopen(fetchurl,cafile=cafile) as response:
      ...
      if fetch == 'json':
        ...
        # case of just returning the json object itself
        mydata=longstring
      else:
        # default is list-- take byte string which we split into a list
  ...
  # returns a list for 'list' or a list of dictionaries for 'json'
...
def SuperMAGGetInventory(logon,start,extent):
  ...
  (success,stations)=sm_GetUrl(urlstr,'list')
...
def SuperMAGGetIndices(logon,start,extent,flagstring='',**kwargs):
  # One of the core 3 functions
  # usual return is 'json' aka pandas frame, but also can just return raw fetch
  typecode = kwargs.get('FORMAT','json').lower()
  ...
  # or even the raw server data
  if typecode == 'list' or typecode == 'raw':
...
def SuperMAGGetData(logon,start,extent,flagstring,stationlist, multi=True, **kwargs):
    # can accept either a single station or multiples
    global grab_multi_data
    if 'multi' in kwargs:
        multi= kwargs.pop('multi')
    def grab_multi_data(stationlist):
        frames=[]
        status=0
        for station in stationlist:
            (tstatus,tdata)=sm_SuperMAGGetData_Single(logon,start,extent,flagstring,station,**kwargs)
            frames.append(tdata)
            status += tstatus
        # done them all so aggregate
        if len(frames)==0:
            return status, None
        else:
            data = pd.concat(frames)
            return status, data
    if type(stationlist) is not list:
        # single item
        (status,data)=sm_SuperMAGGetData_Single(logon,start,extent,flagstring,stationlist,**kwargs)
    else:
        import numpy as np
        stationlist=np.array(stationlist)
        # multiple stations, append into larger dataframe
        if len(stationlist)%8!=0:
            arr=np.split(stationlist[len(stationlist)%7:], 7)
            arr.append(stationlist[:len(stationlist)%7])
        else:
            arr=np.split(stationlist, 8)
        from multiprocessing import Pool
        p= Pool(8)
        if multi:
            try:
                res= np.array(p.map(grab_multi_data, arr), dtype=object)
                status= np.sum(res[:,0])
                frames= res[:,1]
            except:
                multi=False
...
def sm_SuperMAGGetData_Single(logon,start,extent,flagstring,station,**kwargs):
  # One of the core 3 functions
  ...
  # also arg FORMAT='list', otherwise defaults to FORMAT='json'

  # default FORMAT='json' aka dataframe, alt is FORMAT='list' or 'raw'
